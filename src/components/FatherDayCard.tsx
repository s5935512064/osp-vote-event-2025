...
import TextLogo from "../assets/activity_father/father-logo.webp";
...
import FatherDayActivity1 from "../assets/activity_father/ACTIVITY_01.jpg";

import FatherDayCardCreator from "./FatherDayCardCreator";
...
const ContentSection: React.FC<{ onCreateCard: () => void }> = ({
  ...
  <div className="flex flex-col items-center justify-center z-[10] my-2">
    <motion.p
      className="text-gray-600  text-base md:text-lg xl:text-xl max-w-screen-sm text-center leading-relaxed"
      {...animations.text}
    >
      ในโอกาสวันพ่อ 5 ธันวาคมปีนี้ ให้{" "}
    ...
    <motion.button
      ...
      className="mt-4 md:mt-7 bg-[#21252b] text-white md:text-lg font-prompt font-semibold py-3 px-6 rounded-lg transition-colors duration-200 shadow-md cursor-pointer"
      {...animations.button}
    >
      สร้างการ์ดวันพ่อ
    </motion.button>
...
const ActivityCarousel: React.FC = () => {
  ...
  const SLIDES = [
    {
      id: 1,
      src: FatherDayActivity1.src,
      alt: "Father's Day Activity 1",
      link: "https://www.facebook.com/share/p/1D4FsqQF5D/",
    },
  ];
...
const FatherDayCardComponents: React.FC<MothersDayCardProps> = ({}) => {
...
export default FatherDayCardComponents;
